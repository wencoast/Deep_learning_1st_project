...
def inception_resnet_stem(input):
    ...
    # Input Shape is 28 x 28x 1 (tf) or 1x 28 x 28 (th)
    # Default border_model is valid.
    ...
    # Maxpooling 2D default border_mode='valid'
    ...
    c = Convolution2D(256, 3, 3, activation='relu', subsample=(2,2), border_mode='valid')(c)
...
def reduction_A(input, k=192, l=192, m=256, n=384):
    if K.image_dim_ordering() == "th":
        channel_axis = 1
    else:
        channel_axis = -1
...
def create_inception_resnet_v1(nb_classes=1001, scale=True):
    ...
    #x_value = sess.run(x)
    #print('The shape of x_value is', x_value.shape)
