...
csvTrainAcc.append(['epoch(s)','keep_prob='+str(dropout)])
csvTestAcc.append(['epoch(s)','keep_prob='+str(dropout)])
# Launch the graph
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        if (step * batch_size) % display_step == 0:
            ...
            csvTestAcc.append([epoch, test_acc])
            epoch +=1
...
#csvTestAcc = np.reshape(np.ravel(csvTestAcc, order='F'), (-1, 2), order='F')
with open('fig/optimal_testing_acc.csv', 'w') as csvFile:
...
#csvTrainAcc = np.reshape(np.ravel(csvTrainAcc, order='F'), (-1, 2), order='F')

with open('fig/optimal_training_acc.csv', 'w') as csvFile:
    writer = csv.writer(csvFile)
